...
class json:
    ...
    def check_entry_type(entry='', value_type=str, json_data={}):
        flat_json_data = json.flat(json_data)
        for key, value in flat_json_data.items():
            if key == entry:
    ...
    def entry_matches_value(entry='', value=None, json_data={}):
        if json.has_entry(entry=entry, json_data=json_data):
            value_ = json.get_entry_value(entry=entry, json_data=json_data)
            if value_:
                if value == value_:
                    return True
                return False
